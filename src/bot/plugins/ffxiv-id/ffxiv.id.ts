...
const guildLiLiLaUserRepository = LalafellDataSource.getRepository(GuildLiLiLaUser);
const ffxivId = async (
  client: any,
  ...
  if (data.eventType === 'MESSAGE_CREATE' && spread) {
    ...
    if (content?.includes(`<@!${baseConfig.robotId}>`)) {
      if (content?.includes('名片 帮助')) {
        ...
        const help_text = `📘 @bot 名片 帮助 -> 查看名片帮助\n`+
      ...
      if (content?.includes('QAQ')) {
        spread = false; // msg will not be spreaded to other plugins
        const cc = content.replace(`<@!${baseConfig.robotId}>`, '').replace('QAQ', '').trim();
      ...
      box-sizing: border-box;
                  ...
                  <body>
                    <div class="l-box u-transparent u-with-margin b-user-info">
                      <img src="https://${user.avatar}" alt="lilila" class="b-user-info-avatar u-inset"> 
                      <h2 class="b-user-info-text">
                      
                      <span class="b-user-info-text-name ">${user.nickname}</span> 
                      <span class="b-user-info-text-server">«${user.server.replace("."," ")}»</span>
                      </h2>
                    </div>
                  </body>
                </html>`,
        })
        .then(() => {console.log('The image was created successfully!'); postImage(data.msg, './src/bot/plugins/ffxiv-id/card.png');})
      }
